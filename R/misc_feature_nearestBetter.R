...
#'   coefficient of variation of the ratio of the distances and the final
...
calculateNearestBetter = function(feat.object, control) {
  ...
  return(list(
    nn_nb.sd_ratio = sd(nn.dists, na.rm = TRUE) / sd(nb.dists, na.rm = TRUE),
    nn_nb.mean_ratio = mean(nn.dists, na.rm = TRUE) / mean(nb.dists, na.rm = TRUE),
    nn_nb.cor = cor(nn.dists, nb.dists, use = cor_na),
    dist_ratio.coeff_var = 
      sd(dist_ratio, na.rm = TRUE) / mean(dist_ratio, na.rm = TRUE),
    nb_fitness.cor = cor(nb.stats$toMe_count, y, use = cor_na)
  ))
}
