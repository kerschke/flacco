...
test_that("GCM initialisation computations for min are performed correctly", {
  ...
  # empty cells
  expect_equal(length(which(featobj$env$gcm.representatives$min == Inf)), 94)
  
  # number of closed classes
  expect_equal(featobj$env$gcm.canonicalForm$min$closedClassIndex, 92)
  
  # TODO values of featobj$env$gcm.canonicalForm$min$indexPermutation
  # TODO values of featobj$env$gcm.canonicalForm$min$canonicalForm
})
...
test_that("GCM initialisation computations for mean are performed", {
  ...
  # empty cells
  expect_equal(length(which(featobj$env$gcm.representatives$mean == Inf)), 94)
  
  # number of closed classes
  expect_equal(featobj$env$gcm.canonicalForm$mean$closedClassIndex, 91)
  
  # TODO values of featobj$env$gcm.canonicalForm$mean$indexPermutation
  # TODO values of featobj$env$gcm.canonicalForm$mean$canonicalForm
})
