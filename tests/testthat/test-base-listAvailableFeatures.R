...
test_that("listAvailableFeatures", {
  ...
  expect_equal(x, c("cm_angle", "cm_conv", "cm_grad", "ela_conv", "ela_curv",
    ...
    "nbc", "pca", "gcm", "ic"))
#     "nbc", "pca", "gcm", "ic", "barrier_tree"))

#   x = c("barrier_tree", "limo", "ela_cur")
  x = c("limo", "ela_curv")
  ...
  expect_equal(listAvailableFeatureSets(
#     subset = x, allow.additional_costs = FALSE), c("barrier_tree", "limo"))
    subset = x, allow.additional_costs = FALSE), "limo")
